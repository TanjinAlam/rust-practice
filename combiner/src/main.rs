...
fn standardise_size(image_1: DynamicImage, image_2: DynamicImage) -> (DynamicImage, DynamicImage) {
    let (width , hight) = get_smallest_dimension(image_1.dimensions(), image_2.dimensions());
    println!("width: {}, height: {}\n", width, hight);

    if image_2.dimensions() == (width, hight) {
        (image_1.resize_exact(width, hight, Triangle), image_2)
    } else {
        (image_1, image_2.resize(width, hight, Triangle))
    }
}
...
fn combine_images(image_1: DynamicImage, image_2: DynamicImage) -> Vec<u8> {
    ...
    alternate_prixels(vec_1, vec_2)
}

fn alternate_prixels(vec_1: Vec<u8>, vec_2: Vec<u8>)-> Vec<u8> {
